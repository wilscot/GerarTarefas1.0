...
def get_automation_result(execution_id: str):
    ...
    """
    try:
        status = selenium_service.get_process_status(process_id)
        
        if status:
            return jsonify({
                "process_id": process_id,
                "workorder_id": status["workorder_id"],
                "hours_target": status["hours_target"],
                "started_at": status["started_at"].isoformat(),
                "status": status["status"],
                "finished_at": status.get("finished_at", "").isoformat() if status.get("finished_at") else None,
                "return_code": status.get("return_code"),
                "error": status.get("error")
            }), 200
        else:
            return jsonify({
                "error": f"Processo {process_id} não encontrado",
                "process_id": process_id
            }), 404
