...
class UserTasksCacheService:
    ...
    def __init__(self, username: str = "wfrancischini"):
        ...
        # Invalidar apenas o cache de user_tasks para forçar nova query (com correções)
    ...
    def _fetch_user_tasks(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Query simplificada para depuração - Buscar tarefas básicas
            query = """
                SELECT TOP 10
                    td.TASKID as task_number,
                    td.TITLE as title,
                    1 as state,
                    3 as priority,
                    td.CREATEDDATE as opened_at,
                    NULL as resolved_at,
                    0.0 as time_spent,
                    0.0 as time_estimated,
                    'Em Andamento' as state_label,
                    'Não Definida' as priority_label
            ...
            for row_dict in result:
                ...
                if resolved_at and isinstance(resolved_at, (int, float)) and resolved_at > 0:
                    # ACTUALENDTIME está em milissegundos, converter para datetime
                    ...
                    resolved_at = None  # Tarefa ainda não concluída
                
                user_tasks.append({
                    'task_number': row_dict.get('task_number'),
                    'title': row_dict.get('title') or 'Sem título',
                    'state': 3 if resolved_at else 1,  # 3 = Concluída, 1 = Em Andamento
